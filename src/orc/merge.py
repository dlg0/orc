...
# Lockfiles that are routinely regenerated by tooling (e.g. ``uv sync``) and
# can safely be committed or reset before a merge preflight check.
_LOCKFILE_PATTERNS: set[str] = {"uv.lock"}


def _commit_lockfile_noise(cwd: Path, issue_id: str) -> None:
    """Stage and commit lockfile-only changes so they don't block the merge.

    When tools like ``uv`` regenerate ``uv.lock`` during evaluation or
    verification, the worktree becomes dirty even though the change is
    harmless.  This helper detects that case and commits only the lockfile
    changes so the preflight clean-check passes.

    If the dirty files include *anything* besides known lockfiles, this
    function does nothing — the caller's normal dirty-worktree handling
    will kick in.
    """
    # Get raw porcelain output (not stripped) to preserve column layout.
    result = subprocess.run(
        ["git", "status", "--porcelain=v1"],
        cwd=cwd,
        capture_output=True,
    ...
    # Parse porcelain lines — format is "XY <path>" (3-char prefix + path).
    dirty_paths: list[str] = []
    for line in raw_status.splitlines():
        if not line:
            continue
        # Porcelain v1: first two chars are index/worktree status, then a
        # space, then the path.  Renames have " -> newpath" appended.
        path = line[3:].split(" -> ")[-1]
        dirty_paths.append(path)

    if not all(Path(p).name in _LOCKFILE_PATTERNS for p in dirty_paths):
        return  # non-lockfile changes present — leave for caller
    ...
    try:
        ...
        subprocess.run(
            ["git", "commit", "-m", f"chore: update lockfiles for {issue_id}"],
            cwd=cwd,
            check=True,
            capture_output=True,
        )
        logger.info("Auto-committed lockfile changes in %s for %s", cwd, issue_id)
    except subprocess.CalledProcessError:
        # Best-effort — if this fails the normal dirty-check will report it.
        pass


def _git_state_snapshot(cwd: Path) -> dict:
    """Capture a snapshot of the git working-tree state for diagnostics."""
    ...
    for line in porcelain.splitlines():
        ...
        if Path(path).name in _LOCKFILE_PATTERNS:
            lockfile_paths.append(path)

    unmerged = _get_conflict_files(cwd)
    ...
    return {
        ...
        "merge_in_progress": _merge_in_progress(cwd),
        "rebase_in_progress": _rebase_in_progress(cwd),
    }


def _build_merge_diagnostics(
    *,
    reason: str,
    ...
    """Construct a diagnostics dict for merge failures."""
    diag: dict = {"reason": reason}
    if command is not None:
        diag["command"] = command if isinstance(command, str) else " ".join(command)
    if cwd is not None:
        diag["cwd"] = str(cwd)
    if returncode is not None:
        diag["returncode"] = returncode
    if stdout is not None:
        diag["stdout"] = stdout[:2000]
    if stderr is not None:
        diag["stderr"] = stderr[:2000]
    if git_state is not None:
        diag["git_state"] = git_state
    return diag


def _repo_root_preflight(repo_root: Path) -> tuple[bool, dict | None]:
    """Check repo root for dirty tracked files.

    Returns ``(is_clean, diagnostics_or_none)``.  When the repo root is
    clean the second element is ``None``; otherwise it contains a
    diagnostics dict suitable for embedding in a :class:`MergeResult`.
    """
...
def _build_merge_recovery_prompt(
    stage: str,
    error: str,
    base_branch: str,
    issue_id: str,
    ...
    """Build a prompt for agent-driven merge recovery of any merge-stage blocker."""
    diag_section = ""
    if diagnostics:
        reason = diagnostics.get("reason", "unknown")
        cmd = diagnostics.get("command", [])
        ...
        diag_section = "\n".join([
            ...
            "Diagnostics:",
            f"  Reason: {reason}",
            f"  Failed command: {' '.join(str(c) for c in cmd) if cmd else 'N/A'}",
            f"  Return code: {rc}",
        ])
        if stderr:
            diag_section += f"\n  Stderr: {stderr[:500]}"
        if stdout:
            diag_section += f"\n  Stdout: {stdout[:500]}"
        git_state = diagnostics.get("git_state", {})
        if git_state:
            for loc in ("worktree", "repo_root"):
                snap = git_state.get(loc)
                if snap and snap.get("dirty_tracked_paths"):
                    diag_section += f"\n  Dirty tracked files ({loc}): {', '.join(snap['dirty_tracked_paths'][:10])}"

    return "\n".join([
        f"You are recovering from a merge-stage failure for issue {issue_id}.",
        f"The merge of the branch onto `{base_branch}` failed at the `{stage}` stage.",
        f"Error: {error}",
        diag_section,
        "",
        "Your task:",
        f"1. Diagnose and fix the blocker in the worktree at `{worktree_path}`",
        f"2. Rebase the branch onto `origin/{base_branch}`, resolving any conflicts",
        f"3. In the main repo at `{repo_root}`, checkout `{base_branch}` and pull latest",
        f"4. Merge the branch into `{base_branch}` with `--no-ff`",
        f"5. Push to `origin {base_branch}`",
        f"6. Run `bd close {issue_id}`",
        "",
        "Important:",
        "- Inspect the current git state (`git status`, `git log --oneline -5`) before taking action",
        "- If a rebase or merge is in progress, abort it first and start fresh",
        "- If the repo root has dirty tracked files, stash or commit them before merge",
        "- Make sure verification tests pass after resolving the issue",
        "",
        f"Worktree: {worktree_path}",
        f"Main repo: {repo_root}",
    ])


_MERGE_RECOVERY_TIMEOUT = 900  # 15 minutes


def _spawn_merge_recovery_agent(
    worktree_path: Path,
    repo_root: Path,
    stage: str,
    error: str,
    ...
    events: EventLog | None,
    diagnostics: dict | None = None,
    recovery_timeout: int = _MERGE_RECOVERY_TIMEOUT,
    agent_mode: str = "smart",
) -> bool:
    """Spawn an amp agent to recover from any merge-stage failure.

    Returns True if recovery succeeded (postconditions pass).
    """
    amp_path = shutil.which("amp")
    if amp_path is None:
        logger.warning("amp CLI not found — cannot attempt merge recovery")
        return False
    ...
    if events:
        events.record(EventType.conflict_resolution_started, {
            "issue_id": issue_id,
            "stage": stage,
            "recovery_type": "agent_merge_recovery",
        })

    prompt = _build_merge_recovery_prompt(
        stage=stage,
        error=error,
        base_branch=base_branch,
        issue_id=issue_id,
    ...
    logger.info("Spawning merge recovery agent for %s (stage=%s) in %s", issue_id, stage, worktree_path)
    ...
    try:
        ...
        logger.error("Merge recovery agent timed out after %ds", recovery_timeout)
        if events:
            events.record(EventType.conflict_resolution_finished, {
                "issue_id": issue_id,
                "stage": stage,
    ...
    if proc.returncode != 0:
        logger.warning("Merge recovery agent exited with code %d", proc.returncode)
...
def verify_and_merge(
    ...
    # Auto-commit lockfile-only noise (e.g. uv.lock regenerated by evaluation)
    _commit_lockfile_noise(worktree_info.worktree_path, issue_id)
    ...
    if dirty:
        diag = _build_merge_diagnostics(
            ...
            cwd=worktree_info.worktree_path,
            git_state={"worktree": _git_state_snapshot(worktree_info.worktree_path)},
        )
        return MergeResult(
            success=False,
            stage="preflight",
    ...
    # Repo-root preflight: check for dirty tracked files
    repo_clean, repo_diag = _repo_root_preflight(repo_root)
    if not repo_clean:
        return MergeResult(
            success=False,
            stage="preflight",
            error=f"repo root has dirty tracked files",
    ...
    try:
        ...
        if _is_conflict(e):
            ...
            diag = _build_merge_diagnostics(
                ...
                command=["git", "rebase", f"origin/{base_branch}"],
                cwd=worktree_info.worktree_path,
            ...
            return MergeResult(
                success=False, stage="rebase", error=str(e),
                diagnostics=diag, retryable=True,
            )
    ...
    try:
        ...
        diag = _build_merge_diagnostics(
            ...
            stdout=_decode_output(e.stdout),
            stderr=_decode_output(e.stderr),
            git_state={"repo_root": _git_state_snapshot(repo_root)},
        )
        return MergeResult(
            success=False, stage="checkout", error=str(e),
            diagnostics=diag, retryable=True,
        )
    ...
    try:
        ...
        return MergeResult(
            success=False, stage="pull", error=str(e),
            diagnostics=diag, retryable=True,
        )
    ...
    try:
        ...
        if _is_conflict(e):
            ...
            return MergeResult(
                success=False, stage="merge", error=str(e),
                diagnostics=diag, retryable=True,
            )
