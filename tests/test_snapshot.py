...
def test_load_snapshot_defaults(tmp_path: Path) -> None:
    ...
    with patch("orc.tui.snapshot.get_ready_issues", return_value=QueueResult()):
...
def test_load_snapshot_with_state(tmp_path: Path) -> None:
    ...
    with patch("orc.tui.snapshot.get_ready_issues", return_value=QueueResult(issues=[issue])):
        snap = load_snapshot(tmp_path, state_dir)
...
def test_load_snapshot_config_error_uses_default(tmp_path: Path) -> None:
    ...
    with (
        patch("orc.tui.snapshot.get_ready_issues", return_value=QueueResult()),
        patch("orc.tui.snapshot.load_config", side_effect=Exception("bad config")),
    ):
        snap = load_snapshot(tmp_path, state_dir)
